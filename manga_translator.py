...
def _recommend_parallelism(use_gpu: bool, vram_gb: float = 0.0, user_workers: int = 0) -> dict:
    ...
    if use_gpu:
        ...
        page_w = 1 if cpus <= 2 else min(4, max(1, cpus - 1))
        ocr_w = min(8, max(2, cpus))
        chunk_w = 2 if cpus <= 2 else min(4, max(1, cpus - page_w))
        ort_threads = max(1, cpus // max(1, chunk_w)) if chunk_w > 1 else max(1, min(4, cpus))
...
class MangaTranslator:
    ...
    def __init__(
        ...
        debug: bool = False,
        dry_run_fa: bool = False,
        extract_order: str = "auto",
        ...
        self.dry_run_fa = bool(dry_run_fa)
        self._api_dead = False
        self.extract_order = (extract_order or "auto").lower().strip()
    ...
    def detect_bubbles(self, image_bgr: np.ndarray,
                       rescue_edges: Optional[Tuple[str, ...]] = None) -> List[dict]:
        ...
        edges = tuple(rescue_edges) if rescue_edges else ("top", "bottom")
        if edges:
            try:
                boxes = self._edge_strip_rescue(image_bgr, boxes, edges=edges)
            except Exception as e:
                print(f"    [!] edge-rescue خطا: {e}")

        boxes = self._merge_split_bubble_boxes(boxes)
        boxes = self._split_touching_bubbles(boxes, h, w, image_bgr=image_bgr)
        
        
        try:
            boxes = self._drop_contained_boxes(boxes)
            boxes = self._nms_boxes(boxes, 0.45)
        except Exception:
            pass
        return boxes

    @staticmethod
    def _merge_split_bubble_boxes(boxes: List[dict]) -> List[dict]:
        if len(boxes) < 2:
            return boxes

        def _rect(b):
            return [int(v) for v in b["rect"][:4]]

        merged = [dict(b) for b in boxes]
        changed = True
        while changed:
            changed = False
            for i in range(len(merged)):
                ...
                for j in range(i + 1, len(merged)):
                    ...
                    yov = max(0, min(ay2, by2) - max(ay1, by1))
                    hmin = max(1, min(ay2 - ay1, by2 - by1))
                    hmax = max(ay2 - ay1, by2 - by1)
                    if yov < 0.70 * hmin or hmax / hmin > 2.0:
                        continue
                    xov = min(ax2, bx2) - max(ax1, bx1)
                    if xov < 0 and (-xov) > 0.20 * min(ax2 - ax1, bx2 - bx1):
                        continue
                    a["rect"] = [min(ax1, bx1), min(ay1, by1), max(ax2, bx2), max(ay2, by2)]
                    a["confidence"] = max(
                        float(a.get("confidence", 0) or 0), float(b.get("confidence", 0) or 0)
                    )
                    del merged[j]
                    changed = True
                    break
    ...
    @staticmethod
    def _drop_contained_boxes(boxes: List[dict], contain_thresh: float = 0.80) -> List[dict]:
        if len(boxes) < 2:
            return boxes
        ...
        for b in boxes:
            ...
            for k in kept:
                ...
                if smaller > 0 and (ix * iy) / smaller >= contain_thresh:
                    dup = True
                    break
            if not dup:
                kept.append(b)
    ...
    def _edge_strip_rescue(self, image_bgr: np.ndarray, boxes: List[dict],
                           edges: Tuple[str, ...] = ("top", "bottom")) -> List[dict]:
        h, w = image_bgr.shape[:2]
        if h < 400 or w < 120:
            return boxes
    ...
    def _split_touching_bubbles(self, boxes: List[dict], img_h: int, img_w: int,
                                image_bgr: Optional[np.ndarray] = None) -> List[dict]:
        ...
        for b in boxes:
            ...
            elif image_bgr is not None and b.get("class_name") != "text_free":
                
                
                
                try:
                    pm = self._bubble_pixel_mask(image_bgr, rx1, ry1, rx2, ry2,
                                                 rect=(x1, y1, x2, y2))
                except Exception:
                    pm = None
                if pm is not None:
            ...
            ys_m, xs_m = np.where(full > 0)
            mx1, my1 = int(xs_m.min()), int(ys_m.min())
            mx2, my2 = int(xs_m.max()) + 1, int(ys_m.max()) + 1

            comps = self._mask_to_lobes(full, mx1, my1, mx2, my2)
            if len(comps) < 2:
                comps = self._split_by_projection(full, mx1, my1, mx2, my2)

            if len(comps) >= 2:
                parent_a = float(max(1, (full[my1:my2, mx1:mx2] > 0).sum()))
                keep_frac = 0.20 if pixel_mask else 0.30
                comps = [c for c in comps if c[4] >= max(160.0, parent_a * keep_frac)]
                if len(comps) < 2:
                    comps = []

            if len(comps) < 2:
                if pixel_mask:
                    mask_area = float(len(xs_m))
                    bbox_area = float((mx2 - mx1) * (my2 - my1))
                    rect_area = float(max(1.0, (x2 - x1) * (y2 - y1)))
                    
                    
                    
                    if (bbox_area >= 12 * 12
                            and mask_area >= max(400.0, 0.10 * rect_area)
                            and mask_area >= 0.30 * bbox_area):
                        nb = dict(b)
                        pad = 2
                        tx1 = max(0, mx1 + rx1 - pad)
                        ty1 = max(0, my1 + ry1 - pad)
                        tx2 = min(img_w, mx2 + rx1 + pad)
                        ty2 = min(img_h, my2 + ry1 + pad)
                        nb["rect"] = [int(tx1), int(ty1), int(tx2), int(ty2)]
                        cnts, _ = cv2.findContours(full, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
                        if cnts:
                            cnt = max(cnts, key=cv2.contourArea)
                            poly = cnt.reshape(-1, 2).astype(np.int32).copy()
                            poly[:, 0] += rx1
                            poly[:, 1] += ry1
                            nb["mask_poly"] = poly
                            nb["shape_type"] = self._shape_from_contour(cnt)
                        out.append(nb)
                        continue
                out.append(b)
                continue
            ...
            for (xa, ya, xb, yb, _a, lobe_mask) in comps:
                ...
                if lobe_mask is not None:
                    ...
                    if cnts:
                        cnt = max(cnts, key=cv2.contourArea)
                        part_poly = cnt.reshape(-1, 2).astype(np.int32).copy()
                        part_poly[:, 0] += rx1
                        part_poly[:, 1] += ry1
                        part_shape = self._shape_from_contour(cnt)
                nb = dict(b)
                nb["rect"] = [xa, ya, xb, yb]
                
                
                nb["split_parent"] = [x1, y1, x2, y2]
    ...
    @staticmethod
    def _shape_from_contour(cnt) -> str:
        ...
        if circ >= 0.70 and sol >= 0.88:
            return "circle"
        if circ >= 0.40 and sol >= 0.72:
            return "round"
        if sol >= 0.85 and circ < 0.55:
            return "box"
        return "jagged"

    @staticmethod
    def _bubble_pixel_mask(image_bgr: np.ndarray, rx1: int, ry1: int, rx2: int, ry2: int,
                           rect=None, pad: int = 8) -> Optional[np.ndarray]:
        H, W = image_bgr.shape[:2]
        ...
        ch, cw = light.shape
        if rect is not None:
            q1x, q1y, q2x, q2y = rect
            qx1, qy1 = max(0, q1x - px1), max(0, q1y - py1)
            qx2, qy2 = min(cw, q2x - px1), min(ch, q2y - py1)
        else:
            qx1, qy1, qx2, qy2 = 0, 0, cw, ch
        rect_area = float(max(1, (qx2 - qx1) * (qy2 - qy1)))
        min_blob = max(120.0, rect_area * 0.02)

        cands = []
        largest = 0
        for i in range(1, n):
            x, y, w_, h_, a = stats[i, :5]
            if a < min_blob:
                continue
            if x <= 0 or y <= 0 or x + w_ >= cw or y + h_ >= ch:
                continue
            in_rect = float((labels[qy1:qy2, qx1:qx2] == i).sum()) if (qx2 > qx1 and qy2 > qy1) else float(a)
            if in_rect < 0.55 * a:
                continue
            cands.append((i, int(a)))
            largest = max(largest, int(a))
        if largest <= 0:
            return None

        keep = np.zeros_like(light)
        for i, a in cands:
            if a >= max(min_blob, 0.10 * largest):
                keep[labels == i] = 255
        if keep.max() == 0:
            return None
        if float((keep > 0).sum()) > 0.97 * rect_area:
            return None

        
        k = int(min(rx2 - rx1, ry2 - ry1) * 0.02)
        if k % 2 == 0:
            k += 1
        k = max(3, min(7, k))
        ...
        oy1, ox1 = max(0, ry1 - py1), max(0, rx1 - px1)
        oy2 = min(py2 - py1, oy1 + (ry2 - ry1))
        ox2 = min(px2 - px1, ox1 + (rx2 - rx1))
        if oy2 <= oy1 or ox2 <= ox1:
            return None
        canvas[0:oy2 - oy1, 0:ox2 - ox1] = keep[oy1:oy2, ox1:ox2]
        return canvas
            ...
            if mid_hi > mid_lo:
                ...
                if col_s[valley] < 0.25 and col_s[:valley].max() > 0.5 and col_s[valley:].max() > 0.5:
                    ...
                    for part, ox in ((left_mask, 0), (right_mask, valley)):
                        ...
                        if len(xs) < 30:
                            continue
                        full_part = np.zeros_like(mask)
                        full_part[y1:y2, x1 + ox:x1 + ox + part.shape[1]] = part
                        results.append((
                            x1 + ox + int(xs.min()),
                            y1 + int(ys.min()),
                            x1 + ox + int(xs.max()) + 1,
                ...
                if row_s[valley] < 0.25 and row_s[:valley].max() > 0.5 and row_s[valley:].max() > 0.5:
                    for part, oy in ((roi[:valley], 0), (roi[valley:], valley)):
                        ...
                        if len(xs) < 30:
                            continue
                        full_part = np.zeros_like(mask)
                        full_part[y1 + oy:y1 + oy + part.shape[0], x1:x2] = part
                        results.append((
                            x1 + int(xs.min()),
                            y1 + oy + int(ys.min()),
                            x1 + int(xs.max()) + 1,
    ...
    STYLE_LABELS_FA = {
        "normal": "عادی/کودک",
        "shout": "داد خشم/افسانه",
        "comedy_shout": "داد کمدی/کروش",
        "whisper": "زمزمه/دست‌نویس",
        "sun_thought": "خورشید تفکر/مهر",
        "thought": "ابری تفکر/مروارید",
        "free_text": "بیرون‌بالن/ارامکو-هوما-تهران",
        "system": "سیستم/اصفهان-فرناز",
        ...
        "black": "دارک/اتابای-فرزیانی-زنگار",
        "explosion": "انفجاری",
    ...
    _FONT_ALIASES = {
        ...
        "comedy_shout":   ["kroosh", "kurosh", "karush", "krouch", "کروش", "rakkas"],
        ...
        "black":          ["atabay", "atabak", "اتابای", "اتابک", "farziani", "فرزیانی", "zangar", "زنگار", "jomhuria"],
    ...
    def _ocr_crop(self, image_bgr: np.ndarray, rect: List[int], y_offset: int = 0,
                  pad_ratio: float = 0.08, mask_poly=None,
                  force_bottom_px: int = 0, max_variants: int = 0,
                  force_top_px: int = 0,
                  exclude_rects: Optional[List[List[int]]] = None) -> Tuple[str, List[np.ndarray]]:
        ...
        cx1 = max(0, x1 - pad_x)
        cy1 = max(0, y1 - pad_y - max(0, int(force_top_px)))
        cx2 = min(w_img, x2 + pad_x)
        
        cy2 = min(h_img, y2 + pad_y + max(0, int(force_bottom_px)))
        ...
        def _prep_variants(crop_bgr):
            ...
            try:
                ...
                work = cv2.bitwise_not(gray) if mean_v < 115 else gray
                clahe = cv2.createCLAHE(clipLimit=2.0, tileGridSize=(8, 8))
                work_c = clahe.apply(work)
                ...
                if max(h0, w0) < 220:
                    for fx in (3.0, 4.0):
                        up = cv2.resize(otsu, None, fx=fx, fy=fx, interpolation=cv2.INTER_CUBIC)
                        variants.append(cv2.cvtColor(up, cv2.COLOR_GRAY2BGR))
                        up2 = cv2.resize(work_c, None, fx=fx, fy=fx, interpolation=cv2.INTER_CUBIC)
                        variants.append(cv2.cvtColor(up2, cv2.COLOR_GRAY2BGR))
                variants.append(cv2.cvtColor(otsu, cv2.COLOR_GRAY2BGR))
                if mean_v < 115:
                    variants.append(cv2.cvtColor(work_c, cv2.COLOR_GRAY2BGR))
                ...
                if mean_v >= 115:
                    variants.append(cv2.cvtColor(work_c, cv2.COLOR_GRAY2BGR))
                up15 = cv2.resize(work_c, None, fx=1.5, fy=1.5, interpolation=cv2.INTER_CUBIC)
                variants.append(cv2.cvtColor(up15, cv2.COLOR_GRAY2BGR))
            except Exception:
                pass
            return variants
        ...
        def _run_ocr_on(img):
            ...
            m = max(ch, cw)
            if m < 280:
                scale = min(2.5, max(360.0 / float(m), 1.8))
        ...
        def _ocr_result_score(items) -> float:
            ...
            digit_only = len(re.findall(r"\b\d{2,}\b", joined))
        ...
        def _is_good_enough(items, score: float) -> bool:
            ...
            digit_only = len(re.findall(r"\b\d{2,}\b", joined))
            weird = len(re.findall(r"[^\x00-\x7F\s]", joined))
            return (
                avg_conf >= 0.62 and mixed == 0 and digit_only == 0 and weird == 0
                and len(joined.strip()) >= 4 and score >= 50
            )

        best_items = []
        best_score = -1e9
        best_alnum = 0
        variants = _prep_variants(crop0)
        limit = (max_variants + 1) if (max_variants and max_variants > 0) else len(variants)
        for vi, var in enumerate(variants[:limit]):
            if vi == len(variants) - 1 and best_alnum >= 28 and best_score >= 55:
                break
            try:
            ...
            if score > best_score:
                best_score = score
                best_items = items
                best_alnum = len(re.sub(r"[^\w]", "", " ".join(t for _, t, _ in items), flags=re.UNICODE))

            if _is_good_enough(items, score):
                break
            if best_score >= 60 and len(items) >= 2:
                break
        ...
        if exclude_rects:
            def _owned_by_other(poly) -> bool:
                ...
                if p.size < 4:
                    return False
                p[:, 1] -= y_offset
                lx1, ly1 = float(p[:, 0].min()), float(p[:, 1].min())
                lx2, ly2 = float(p[:, 0].max()), float(p[:, 1].max())
                la = max(1.0, (lx2 - lx1) * (ly2 - ly1))
                cx = (lx1 + lx2) / 2.0
                cy = (ly1 + ly2) / 2.0
                for ex in exclude_rects:
                    try:
                        ex1, ey1, ex2, ey2 = [float(v) for v in list(ex)[:4]]
                    except Exception:
                        continue
                    if ex2 - ex1 < 8 or ey2 - ey1 < 8:
                        continue
                    ix = max(0.0, min(lx2, ex2) - max(lx1, ex1))
                    iy = max(0.0, min(ly2, ey2) - max(ly1, ey1))
                    if (ix * iy) / la < 0.50:
                        continue
                    if (ex1 + 2.0) <= cx <= (ex2 - 2.0) and (ey1 + 2.0) <= cy <= (ey2 - 2.0):
                        return True
                return False

            _filtered = [it for it in best_items if not _owned_by_other(it[0])]
            if len(_filtered) != len(best_items):
                best_items = _filtered
        ...
        for ln, pl in zip(lines, polys):
            ln = ln.strip()
            if not ln:
                continue
            if deduped and deduped[-1][0].lower() == ln.lower():
                continue
            ...
            if deduped:
                ...
                if ln.lower() in prev.lower() or prev.lower() in ln.lower():
                    if len(ln) > len(prev):
                        deduped[-1] = (ln, pl)
                    continue
            deduped.append((ln, pl))
        lines = [t for t, _ in deduped]
        polys = [p for _, p in deduped]
    ...
    _WM_PATTERNS = (
        re.compile(r"\bLIKE\s?MA(?:NGA)?\b[.,!]?\s*(?:[.,]?\s*I\s?N[IK]\b[.,!]?)?", re.I),
        re.compile(r"\bMANGA\s*[.,]?\s*I\s?N[IK]\b[.,!]?", re.I),
        re.compile(r"MANGA\s*[.,]?\s*I\s?N[IK]\b[.,!]?", re.I),  
        re.compile(r"(?<=\w)MANGA\b[.,!]*\s*"),  
        re.compile(r"\bVORTEX\s?SCANS\b[.,!]?\s*(?:[.,]?\s*(?:COM|ORG)\b[.,!]?)?", re.I),
        re.compile(r"\bMGEKO\b[.,!]*", re.I),
    )

    @classmethod
    def _strip_watermark_tokens(cls, text: str) -> str:
        if not text:
            return text
        out = text
        for p in cls._WM_PATTERNS:
            out = p.sub(" ", out)
        out = re.sub(r"[ \t]{2,}", " ", out).strip(" \t|,")
        
        out = re.sub(r"\s+\bLIKE\s*MA\w{0,4}\s*$", "", out, flags=re.I)
        return out.strip()
    ...
    def _classify_text(text: str) -> str:
        ...
        for w in WATERMARK_PATTERNS:
            ...
            if re.search(r"(?<![a-z0-9])" + re.escape(w.replace(" ", "")) + r"(?![a-z0-9])", low_compact):
                return "promo"
    ...
    def _build_text_mask(self, image: np.ndarray, regions: List[TextRegion]) -> np.ndarray:
        ...
        for region in regions:
            ...
            try:
                n_lab, n_lbl, n_stats, _ = cv2.connectedComponentsWithStats(ink, connectivity=8)
                if n_lab > 1:
                    ink_keep = np.zeros_like(ink)
                    ch_b, cw_b = ink.shape[:2]
                    for li in range(1, n_lab):
                        la = int(n_stats[li, cv2.CC_STAT_AREA])
                        lx = int(n_stats[li, cv2.CC_STAT_LEFT])
                        ly = int(n_stats[li, cv2.CC_STAT_TOP])
                        lw = int(n_stats[li, cv2.CC_STAT_WIDTH])
                        lh = int(n_stats[li, cv2.CC_STAT_HEIGHT])
                        touches = (
                            lx <= 1 or ly <= 1
                            or lx + lw >= cw_b - 1 or ly + lh >= ch_b - 1
                        )
                        big = max(lw, lh) >= 0.55 * max(ch_b, cw_b)
                        
                        
                        frame = (lw >= 0.70 * cw_b) and (lh >= 0.70 * ch_b)
                        if frame or (touches and big) or la >= 0.30 * ch_b * cw_b:
                            continue
                        ink_keep[n_lbl == li] = 255
                    ink = ink_keep
            except Exception:
                pass
    ...
    def clean_image(self, image: np.ndarray, regions: List[TextRegion]) -> np.ndarray:
            ...
            if region.boxes:
                ...
                if all_pts:
                    ...
                    if pts.size >= 4:
                        ...
                        bx2 = float(pts[:, 0].max())
                        by2 = float(pts[:, 1].max())
                        _inter = max(0.0, min(bx2, x2) - max(bx1, x1)) * max(0.0, min(by2, y2) - max(by1, y1))
                        _union = (bx2 - bx1) * (by2 - by1) + (x2 - x1) * (y2 - y1) - _inter
                        _poly_a = max(1.0, (bx2 - bx1) * (by2 - by1))
                        _rect_a = max(1.0, (x2 - x1) * (y2 - y1))
                        _contain = _inter / _poly_a
                        
                        
                        
                        if _union > 1.0 and (
                            (_inter / _union) >= 0.25
                            or (_contain >= 0.70 and _poly_a <= _rect_a * 1.5)
                        ) and (bx2 - bx1) >= 4 and (by2 - by1) >= 4:
                            
                            
                            
                            if (_inter / _union) >= 0.25 and _poly_a >= 0.45 * _rect_a:
        ...
        def _build_local_mask(crop, region, x0, y0, edge_top=False, edge_bot=False):
            ...
            if getattr(region, "kind", "dialogue") == "promo":
                try:
                    if float((crop_gray >= 170).mean()) < 0.55:
                        return None
                except Exception:
                    pass
            if region.boxes:
            ...
            if med > 135:
                ink = (crop_gray < med - 16).astype(np.uint8) * 255
            else:
                ink = (crop_gray > med + 16).astype(np.uint8) * 255
            try:
                hsv = cv2.cvtColor(crop, cv2.COLOR_BGR2HSV)
            ...
            frame_mask = np.zeros((ch, cw), dtype=np.uint8)
            try:
                n_lab, n_lbl, n_stats, _ = cv2.connectedComponentsWithStats(ink, connectivity=8)
                if n_lab > 1:
                    ink_keep = np.zeros_like(ink)
                    for li in range(1, n_lab):
                        ...
                        touches = (
                            lx <= 1 or ly <= 1
                            or lx + lw >= cw - 1 or ly + lh >= ch - 1
                        )
                        big = max(lw, lh) >= 0.55 * max(ch, cw)
                        
                        
                        frame = (lw >= 0.70 * cw) and (lh >= 0.70 * ch)
                        if frame or (touches and big) or la >= 0.30 * ch * cw:
                            if frame or (touches and big):
                                frame_mask[n_lbl == li] = 255
                            continue
                        ink_keep[n_lbl == li] = 255
                    ink = ink_keep
            except Exception:
            ...
            if filled:
                ...
                _jagged = (getattr(region, "shape_type", "") or "").lower() == "jagged"
                
                
                try:
                    pz = cv2.dilate(local_mask, np.ones((5, 5), np.uint8), iterations=2)
                    ring = cv2.bitwise_and(pz, cv2.bitwise_not(local_mask))
                    if np.any(ring > 0):
                        _rv = crop_gray[ring > 0]
                        bright_frac = float((_rv >= 170).mean())
                        ring_std = float(_rv.std())
                    else:
                        bright_frac = 1.0
                        ring_std = 0.0
                except Exception:
                ...
                if (not _jagged) and bright_frac >= 0.55 and ring_std <= 22:
                    local_mask = cv2.bitwise_or(local_mask, cv2.bitwise_and(ink, near))
                else:
                    
                    
                    if getattr(region, "rescued_caption", False) and not _jagged:
                        pass  
                    else:
                        
                        
                        _zone_g = local_mask if _jagged else near
                        glyph = cv2.bitwise_and(ink, _zone_g)
                        try:
                            white_ink = ((crop_gray >= 215).astype(np.uint8)) * 255
                            white_ink = cv2.bitwise_and(white_ink, local_mask)
                            glyph = cv2.bitwise_or(glyph, white_ink)
                        except Exception:
                            pass
                        glyph = cv2.morphologyEx(glyph, cv2.MORPH_CLOSE, k2, iterations=1)
                        if np.count_nonzero(glyph) >= max(60, int(0.01 * max(1, np.count_nonzero(local_mask)))):
                            local_mask = glyph
                        
                    if getattr(region, "rescued_caption", False):
                        
                        
                        try:
                            _rx, _ry, _rw, _rh = [int(v) for v in getattr(region, "rect", (0, 0, 0, 0))]
                            _mx = max(6, int(0.12 * _rw))
                            _my = max(6, int(0.12 * _rh))
                            _zx0 = max(0, _rx - x0 - _mx)
                            _zx1 = min(cw, _rx - x0 + _rw + _mx)
                            _zy0 = max(0, _ry - y0 - _my)
                            _zy1 = min(ch, _ry - y0 + _rh + _my)
                            if _zx1 > _zx0 and _zy1 > _zy0:
                                _b, _g, _r = crop[..., 0], crop[..., 1], crop[..., 2]
                                yellow = ((_b < 100) & (_g > 145) & (_r > 165)).astype(np.uint8) * 255
                                zone_y = np.zeros_like(local_mask)
                                zone_y[_zy0:_zy1, _zx0:_zx1] = 255
                                yellow = cv2.bitwise_and(yellow, zone_y)
                                yellow = cv2.morphologyEx(yellow, cv2.MORPH_OPEN, k2, iterations=1)
                                yellow = cv2.dilate(yellow, np.ones((5, 5), np.uint8), iterations=1)
                                local_mask = cv2.bitwise_or(local_mask, yellow)
                        except Exception:
                            pass
                ...
                try:
                    r_ = getattr(region, "rect", (0, 0, 0, 0))
                    rh_ = max(1, int(r_[3]))
                    ry0e = max(0, int(r_[1]) - y0 - int(0.12 * rh_))
                    ry1e = min(ch, int(r_[1] + r_[3]) - y0 + int(0.12 * rh_))
                    ys_ocr, xs_ocr = np.where(local_mask > 0)
                    if ys_ocr.size and ry1e > ry0e:
                        y_top_ocr, y_bot_ocr = int(ys_ocr.min()), int(ys_ocr.max())
                        x_l_ocr, x_r_ocr = int(xs_ocr.min()), int(xs_ocr.max())
                        line_h = max(12, y_bot_ocr - y_top_ocr)
                        
                        if med > 135:
                            bgm = (crop_gray >= max(170, med - 25))
                        else:
                            bgm = (crop_gray <= med + 25)
                        bgm = bgm.astype(np.uint8) * 255
                        k_bg = cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (21, 21))
                        bgm = cv2.morphologyEx(bgm, cv2.MORPH_CLOSE, k_bg)
                        bgm = cv2.erode(bgm, np.ones((5, 5), np.uint8))
                        bgm = cv2.dilate(bgm, np.ones((9, 9), np.uint8))
                        zr = ink[ry0e:ry1e]
                        rows_any = (zr > 0).any(axis=1)
                        bands = []
                        in_b = False
                        gap = 0
                        b0 = 0
                        for i, v in enumerate(rows_any):
                            if v:
                                if not in_b:
                                    b0 = i
                                    in_b = True
                                ...
                                gap += 1
                                if gap > 8:
                                    bands.append((b0, i - gap + 1))
                                    in_b = False
                        if in_b:
                            bands.append((b0, len(rows_any)))
                        for (b0, b1) in bands:
                            if b1 < y_top_ocr - int(0.8 * line_h) or b0 > y_bot_ocr + int(0.8 * line_h):
                                continue
                            seg = zr[b0:b1]
                            nz = np.where((seg > 0).any(axis=0))[0]
                            if nz.size == 0:
                                continue
                            c0, c1 = int(nz.min()), int(nz.max()) + 1
                            bw_ = c1 - c0
                            ov = max(0, min(c1, x_r_ocr) - max(c0, x_l_ocr))
                            if bw_ <= 0 or ov < 0.30 * bw_:
                                continue
                            
                            bseg = ink[ry0e + b0:ry0e + b1, c0:c1]
                            ring = cv2.dilate(bseg, np.ones((9, 9), np.uint8))
                            ring = cv2.bitwise_and(ring, cv2.bitwise_not(bseg))
                            rn = int(np.count_nonzero(ring))
                            if rn < 40:
                                continue
                            wfrac = float(np.count_nonzero(cv2.bitwise_and(ring, bgm[ry0e + b0:ry0e + b1, c0:c1]))) / float(rn)
                            if wfrac < 0.5:
                                continue
                            local_mask[ry0e + b0:ry0e + b1, c0:c1] = cv2.bitwise_or(
                                local_mask[ry0e + b0:ry0e + b1, c0:c1], bseg
                            )
                except Exception:
                    pass
            else:
                kind_ = getattr(region, "kind", "dialogue")
                r_ = getattr(region, "rect", (0, 0, 0, 0))
                rx0 = max(0, int(r_[0]) - x0)
                ry0 = max(0, int(r_[1]) - y0)
                rx1 = min(cw, int(r_[0] + r_[2]) - x0)
                ry1 = min(ch, int(r_[1] + r_[3]) - y0)
                if rx1 > rx0 and ry1 > ry0:
                    if kind_ in ("promo", "sfx"):
                        ...
                        zone = np.zeros_like(local_mask)
                        zone[ry0:ry1, rx0:rx1] = 255
                        local_mask = cv2.bitwise_and(ink, zone)
                        filled = bool(np.any(local_mask))
            ...
            try:
                if np.any(frame_mask) and (getattr(region, "shape_type", "") or "").lower() != "jagged":
                    fm = cv2.dilate(frame_mask, np.ones((5, 5), np.uint8), iterations=1)
                    near_zone = cv2.dilate(
                        cv2.bitwise_or(local_mask, ink), np.ones((7, 7), np.uint8), iterations=1
                    )
                    fm = cv2.bitwise_and(fm, cv2.bitwise_not(near_zone))
                    local_mask = cv2.bitwise_and(local_mask, cv2.bitwise_not(fm))
            except Exception:
                pass
            local_mask = cv2.dilate(local_mask, k2, iterations=1)
            if os.environ.get("DUMP_MASKS"):
                try:
                    _dd = os.environ["DUMP_MASKS"]
                    os.makedirs(_dd, exist_ok=True)
                    _dn = len([f for f in os.listdir(_dd) if f.startswith("mask_")])
                    _tag = f"r{int(getattr(region, 'id', -1))}"
                    cv2.imwrite(
                        os.path.join(_dd, f"mask_{_dn:03d}_{_tag}.png"),
                        cv2.cvtColor(crop, cv2.COLOR_BGR2GRAY) * (local_mask > 0),
                    )
                    cv2.imwrite(os.path.join(_dd, f"crop_{_dn:03d}_{_tag}.png"), crop)
                    ov = crop.copy()
                    ov[local_mask > 0] = (0, 0, 255)
                    cv2.imwrite(os.path.join(_dd, f"ovl_{_dn:03d}_{_tag}.png"), ov)
                except Exception:
                    pass
            return local_mask
        ...
        def _smart_fill(crop: np.ndarray, mask: np.ndarray) -> Optional[np.ndarray]:
            ...
            if bg_med < 135:
                ...
                if y0r > 4:
                    t0b = max(0, y0r - band)
                    top_cols = np.median(crop[t0b:y0r, :, :], axis=0).astype(np.float32)  
                else:
                    top_cols = None
                if y1r < h_ - 5:
        ...
        def _residual_text_in_crop(result_crop: np.ndarray, crop: np.ndarray, msk: np.ndarray) -> np.ndarray:
            ...
            try:
                items = self._ocr_crop(
                    result_crop, [0, 0, result_crop.shape[1], result_crop.shape[0]],
                    pad_ratio=0.0, max_variants=2,
                )
        ...
        def _ink_ratio_before_after(orig_crop, result_crop, msk):
            try:
                g0 = cv2.cvtColor(orig_crop, cv2.COLOR_BGR2GRAY)
                g1 = cv2.cvtColor(result_crop, cv2.COLOR_BGR2GRAY)
                ...
                if med0 > 135:
                    ...
                    ink1 = (g1 > med0 + 22)
                m = msk > 0
                if not np.any(m) or not np.any(ink0[m]):
                    return 0.0
                return float(np.count_nonzero(ink1[m])) / float(np.count_nonzero(ink0[m]))
            except Exception:
                return 0.0
        ...
        def _process_one(job):
            ...
            def _kill_letter_ghosts(filled: np.ndarray, msk: np.ndarray) -> np.ndarray:
                try:
                    g1 = cv2.cvtColor(filled, cv2.COLOR_BGR2GRAY)
                    m = msk > 0
                    n_m = int(np.count_nonzero(m))
                    if n_m < 40:
                        return filled
                    med_m = float(np.median(g1[m]))
                    
                    zone = cv2.dilate(msk, np.ones((3, 3), np.uint8), iterations=1)
                    
                    
                    if med_m > 135:
                        resid = (g1 < med_m - 12) & (zone > 0)
                    else:
                        resid = (g1 > med_m + 12) & (zone > 0)
                    n_res = int(np.count_nonzero(resid))
                    if n_res <= max(20, int(0.005 * n_m)):
                        return filled
                    rm = cv2.dilate(resid.astype(np.uint8) * 255, np.ones((7, 7), np.uint8), iterations=2)
                    rm = cv2.bitwise_and(rm, cv2.dilate(msk, np.ones((3, 3), np.uint8), iterations=6))
                    if not np.any(rm):
                        return filled
                    return _opencv_inpaint(filled, rm)
                except Exception:
                    return filled
            ...
            try:
                if use_fast:
                    ...
                    _ink_ratio = _ink_ratio_before_after(crop, filled, local_mask)
                    if _ink_ratio > 0.25:
                        big_m = cv2.dilate(local_mask, np.ones((3, 3), np.uint8), iterations=2)
                        filled = _opencv_inpaint(crop, big_m)
                        print(f"    [OpenCV] residual -> opencv redo (id={rid})", flush=True)
                    elif _ink_ratio > 0.08:
                        filled = _residual_text_in_crop(filled, crop, local_mask)
                    if (getattr(region, "shape_type", "") or "").lower() != "jagged":
                        filled = _kill_letter_ghosts(filled, local_mask)
                    result_crop = filled
                    
                    
                    paste_m = cv2.dilate(local_mask, np.ones((3, 3), np.uint8), iterations=2)
                    out = _feather_paste_pair(result_crop, crop, paste_m)
                ...
                _ink_ratio = _ink_ratio_before_after(crop, lama_out, local_mask)
                if _ink_ratio > 0.25:
                    big_m = cv2.dilate(local_mask, np.ones((3, 3), np.uint8), iterations=2)
                    lama_out = _opencv_inpaint(crop, big_m)
                    print(f"    [{getattr(self, '_inpainter_name', 'LaMa')}] residual -> opencv redo (id={rid})", flush=True)
                elif _ink_ratio > 0.08:
                    lama_out = _residual_text_in_crop(lama_out, crop, local_mask)
                lama_out = _kill_letter_ghosts(lama_out, local_mask) if (
                    (getattr(region, "shape_type", "") or "").lower() != "jagged"
                ) else lama_out
                paste_m = cv2.dilate(local_mask, np.ones((3, 3), np.uint8), iterations=2)
                out = _feather_paste_pair(lama_out, crop, paste_m)
    ...
    _TONE_ALIASES = {
        ...
        "rage": "shout", "furious": "shout",
        "comedy": "comedy_shout", "funny": "comedy_shout", "comic": "comedy_shout", "gag": "comedy_shout",
        "whispering": "whisper", "soft": "whisper", "murmur": "whisper",
        "crying": "cry", "sobbing": "cry", "tears": "cry",
        "scared": "fear", "terror": "fear", "afraid": "fear",
        "beast": "monster", "roar": "monster", "growl": "monster",
        "device": "broadcast", "radio": "broadcast", "phone": "broadcast",
        "tv": "broadcast", "television": "broadcast", "call": "broadcast", "mobile": "broadcast",
        "scroll": "letter", "mail": "letter", "note": "letter",
        "thinking": "thought", "cloud": "thought", "cloudy": "thought",
        "cloud_thought": "thought", "thought_cloud": "thought",
        "sun": "sun_thought", "sunny": "sun_thought", "sunburst": "sun_thought",
        "square": "square_thought", "box_thought": "square_thought",
        "dark": "black", "evil": "black",
        "ui": "system", "status": "system", "hud": "system",
        "caption": "free_text", "outside": "free_text", "overlay": "free_text",
        "caption_box": "narrator",
        "regular": "normal", "dialogue": "normal",
    }
    ...
    def _fuse_bubble_style(self, visual: str, tone: str) -> str:
        ...
        if visual in ("free_text", "sfx_shape", "sfx"):
            return visual
    ...
    def _parse_translation_response(self, text: str, regions: List[TextRegion]) -> bool:
        ...
        for item in results:
            ...
            src_fix = (item.get("source") or item.get("ocr") or "").strip()
            if src_fix:
                source_by_id[item["id"]] = src_fix
        applied = 0
        for region in regions:
            src_fix = source_by_id.get(region.id, "").strip()
            if src_fix and src_fix != (region.source_text or "").strip():
                old = (region.source_text or "")[:60]
                region.source_text = src_fix
                print(f"    [OCR-fix] #{region.id}: {old!r} → {src_fix[:60]!r}")
            ...
            if t:
                ...
                if tone:
                    ...
                    if fused != old_st:
                        fa = self.STYLE_LABELS_FA.get(fused, fused)
                        print(f"    [سبک/AI] #{region.id}: {old_st} ← {tone} → {fused} ({fa})")
    ...
    def _translate_with_gemini(self, user_prompt: str, system_instruction: str,
        ...
        if page_image_bgr is not None and genai_types is not None:
            try:
                img = page_image_bgr
                h, w = img.shape[:2]
                max_w = 1600
                ...
                if ok:
                    ...
                    print(f"    [*] Vision: تصویر صفحه ارسال شد ({len(jpeg_bytes)//1024}KB) — OCR کامل + تشخیص نوع بالن")
            except Exception as e:
                print(f"    [!] آماده‌سازی تصویر vision ناموفق ({e}) → فقط متن")
                contents = user_prompt
    ...
    _FA_TEST_TEXTS = {
        "normal": "این یک متن آزمایشی فارسی است.",
        "shout": "چه خبر؟! باورم نمیشه!",
        "comedy_shout": "وای! چه سوتی دادی! هه هه!",
        "whisper": "آروم باش... دارم می‌شنوم.",
        "cry": "نمی‌تونم باور کنم... اینجوری تموم شد...",
        "fear": "این صدا... چیزی داره میاد!",
        "monster": "خاموش شو، کرم کوچک!",
        "system": "اعلان سیستم: سطح خطر در بالاترین حد است.",
        "broadcast": "پیام همگانی: همه به پناهگاه منتقل شوند.",
        "letter": "نامه‌ای از گذشته، برای تو رسیده است.",
        "thought": "پس این همونه... بالاخره پیداش کردم.",
        "dark": "تاریکی، همه‌جا را فرا گرفت.",
    }

    def _fake_translate_regions(self, regions: List[TextRegion]) -> None:
        print("[TEST] حالت تست فارسی (بدون API)")
        for r in regions:
            tone = (getattr(r, "bubble_style", "") or "normal")
            if tone not in self._FA_TEST_TEXTS:
                tone = "normal"
            r.tone = tone
            r.translated_text = self._FA_TEST_TEXTS.get(tone, f"متن آزمایشی شمارهٔ {r.id}.")
        print(f"[TEST] {len(regions)} ناحیه با متن فارسی آزمایشی پر شد.")

    def translate_regions(self, regions: List[TextRegion], page_image_bgr=None) -> None:
        if not regions:
            return
        if getattr(self, "_api_dead", False):
            return
        if getattr(self, "dry_run_fa", False):
            self._fake_translate_regions(regions)
            return
        if page_image_bgr is None:
            page_image_bgr = getattr(self, "_current_page_image", None)

        payload = [
            {
                "id": r.id,
                "text": r.source_text,
                "bubble_hint": (getattr(r, "bubble_style", "") or "normal"),
                "box": list(getattr(r, "rect", (0, 0, 0, 0))[:4]),
            }
            for r in regions
        ]
        ...
        if page_image_bgr is not None:
            vision_note = (
                "تصویر کامل صفحه همراه این پیام است.\n"
                "۱) برای هر آیتم متن داخل حباب را از تصویر بخوان و در «source» بنویس (کامل و صحیح).\n"
                "۲) نوع/سبک بالن را از ظاهر حباب در تصویر تشخیص بده و در «tone» بگذار.\n"
                "فیلد text فقط OCR تقریبی است؛ فیلد bubble_hint فقط حدس ضعیف کد است — تصمیم نهایی با توست.\n"
                "box=[x,y,w,h] موقعیت تقریبی حباب است.\n\n"
            )

        user_prompt = (
            "این‌ها دیالوگ‌های یک صفحه مانهوا هستند.\n\n"
            + vision_note +
            "متن OCR ممکن است ناقص/خراب باشد (مثل goo7→good، PART TOF→PART OF). "
            "اول source انگلیسی کامل را بساز، بعد ترجمه طبیعی فارسی.\n\n"
            "━━━ جدول سبک بالن (tone) — باید یکی از این‌ها باشد ━━━\n"
            "normal         → بالن عادی گرد/بیضی سفید (کودک)\n"
            "shout          → داد خشم، لبه دندانه‌دار/منفجر (افسانه)\n"
            "comedy_shout   → داد کمدی، لبه خیلی دندانه‌دار (کروش)\n"
            "whisper        → زمزمه، حباب کوچک موج‌دار (دست‌نویس)\n"
            "sun_thought    → تفکر خورشیدی با پرتو دور حباب (مهر)\n"
            "thought        → تفکر ابری با حبابک‌های دنباله (مروارید)\n"
            "free_text      → متن آزاد بیرون بالن روی صفحه (ارامکو/هوما/تهران)\n"
            "system         → باکس سیستم/UI (اصفهان/فرناز)\n"
            "monster        → صدای هیولا، لبه خشن تیره (کردی)\n"
            "cry            → گریه/بغض، موج نرم (موج/هاله)\n"
            "fear           → ترس لرزان (صحرا)\n"
            "broadcast      → بی‌سیم/تلویزیون/تماس موبایل (اکبر/اسمان/مثلث)\n"
            "letter         → نامه/طومار کاغذی (آندالوس/فورات)\n"
            "narrator       → مستطیل راوی (الهام)\n"
            "square_thought → فکر مربعی/زاویه‌دار (یکان)\n"
            "black          → دارک، حباب تیره/پس‌زمینه مشکی (اتابای/فرزیانی/زنگار)\n\n"
            "اگر ظاهر حباب با bubble_hint فرق داشت، ظاهر تصویر را مبنا بگیر.\n"
            "لحن روی خود ترجمه هم اثر بگذارد.\n\n"
            + gloss_note +
            "فقط JSON: [{id, source, translation, tone, names?}]\n\n"
            f"{json.dumps(payload, ensure_ascii=False, indent=2)}"
        )
        self._translate_page_image = page_image_bgr
        delay = 3.0
        last_err = None
                ...
                def _do_call():
                    if self.provider_type == "gemini":
                        return self._translate_with_gemini(
                            ...
                            page_image_bgr=getattr(self, "_translate_page_image", None),
                        )
    ...
    def _render_box_for_region(self, region: TextRegion, img_w: int, img_h: int) -> Tuple[int, int, int, int]:
        ...
        shape = (getattr(region, "shape_type", "") or "").lower()
        if shape != "box" and (getattr(region, "det_class", "text_bubble") or "text_bubble") == "text_bubble":
            ew = max(16, int(w * 0.78))
            eh = max(16, int(h * 0.86))
            ex = x + (w - ew) // 2
            ey = y + (h - eh) // 2
            pad_e = max(3, int(min(ew, eh) * 0.05))
            return (ex + pad_e, ey + pad_e, max(16, ew - 2 * pad_e), max(16, eh - 2 * pad_e))
    ...
    def _split_by_ocr_clusters(
        ...
        if not polys or len(polys) < 2 or not line_texts:
            cls._split_rejected_overlap = False
            return []
        ...
        if len(parts) >= 2:
            ...
            def _ov_frac(a, b):
                ...
                amin = max(1.0, min(aw * ah, bw * bh))
                return (ix * iy) / amin
            for i in range(len(parts)):
                for j in range(i + 1, len(parts)):
                    if _ov_frac(parts[i], parts[j]) > 0.12:
                        cls._split_rejected_overlap = True
                        return []
        cls._split_rejected_overlap = False
    ...
    def _process_chunk_worker(self, args_tuple) -> List[TextRegion]:
        ...
        if len(args_tuple) >= 5:
            idx, y0, y1, image, rescue_edges = args_tuple[:5]
        else:
            idx, y0, y1, image = args_tuple[:4]
            rescue_edges = ("top", "bottom")
        print(f"    [>] تشخیص حباب + OCR تیکه‌ی {idx + 1} (ردیف {y0} تا {y1})")
        piece = image[y0:y1, :]

        bubble_boxes = self.detect_bubbles(piece, rescue_edges=rescue_edges)
        print(f"        تشخیص خام: {len(bubble_boxes)} باکس")

        
        
        _piece_h, _piece_w = piece.shape[:2]

        def _x_ov_px(a, b):
            return max(0, min(a[2], b[2]) - max(a[0], b[0]))

        bottom_room: Dict[int, int] = {}
        top_room: Dict[int, int] = {}
        for _i, _d in enumerate(bubble_boxes):
            _bx1, _by1, _bx2, _by2 = _d["rect"]
            _room = min(90, int(0.45 * (_by2 - _by1)))
            _limit = _by2 + _room
            for _j, _o in enumerate(bubble_boxes):
                if _j == _i:
                    continue
                _ox1, _oy1, _ox2, _oy2 = _o["rect"]
                if _ox1 >= _bx2 or _ox2 <= _bx1:
                    continue
                if _x_ov_px(_d["rect"], _o["rect"]) < 0.45 * min(_bx2 - _bx1, _ox2 - _ox1):
                    continue
                if _oy1 >= _by2 - 6:
                    _limit = min(_limit, _oy1 - 4)
            _limit = min(_limit, _piece_h)
            bottom_room[_i] = max(0, int(_limit - _by2))

            
            
            
            _room_t = min(90, int(0.45 * (_by2 - _by1)))
            _limit_t = _by1 - _room_t
            for _j, _o in enumerate(bubble_boxes):
                if _j == _i:
                    continue
                _ox1, _oy1, _ox2, _oy2 = _o["rect"]
                ...
                if _x_ov_px(_d["rect"], _o["rect"]) < 0.45 * min(_bx2 - _bx1, _ox2 - _ox1):
                    continue
                if _oy2 <= _by1 + 6:
                    _limit_t = max(_limit_t, _oy2 + 4)
            _limit_t = max(0, _limit_t)
            top_room[_i] = max(0, int(_by1 - _limit_t))
        ...
        def _run_ocr_det(_box_idx, det):
            try:
                excl = [o["rect"] for _j, o in enumerate(bubble_boxes) if _j != _box_idx]
                ocr_out = self._ocr_crop(
                    piece, det["rect"], y_offset=y0, mask_poly=det.get("mask_poly"),
                    force_bottom_px=int(bottom_room.get(_box_idx, 0)),
                    force_top_px=int(top_room.get(_box_idx, 0)),
                    exclude_rects=excl,
                )
                if len(ocr_out) == 3:
        ...
        if n_boxes == 0:
            ...
            for i, det in enumerate(bubble_boxes):
                ocr_results.append(_run_ocr_det(i, det))
            ...
            with ThreadPoolExecutor(max_workers=w) as ex:
                futs = {ex.submit(_run_ocr_det, i, det): i for i, det in enumerate(bubble_boxes)}
                tmp = [None] * n_boxes
                done_n = 0
        ...
        for det, text, polys, line_texts in ocr_results:
            ...
            if getattr(self, "debug", False):
                _show = (text or "").replace("\n", " ")[:90]
                print(f"        [dbg-ocr] box=({x1},{y1b},{x2},{y2b}) conf={conf:.2f} len={len(text or '')} { _show!r }")
            if not text:
                empty_ocr += 1
                if getattr(self, "debug", False):
                    print(f"        [dbg-drop] reason=EMPTY box=({x1},{y1b},{x2},{y2b}) conf={conf:.2f}")
                continue
            ...
            if self._is_weak_ocr_text(text):
                empty_ocr += 1
                if getattr(self, "debug", False):
                    print(f"        [dbg-drop] reason=WEAK {text!r}")
                continue
            ...
            _alnum_txt = re.sub(r"[^\w]", "", text, flags=re.UNICODE)
            _words3 = re.findall(r"[A-Za-z]{3,}", text)
            _looks_real_text = (
                len(re.findall(r"[A-Za-z]", text)) >= 8
                and len(_words3) >= 2
                and (sum(len(w) for w in _words3) / max(1, len(_alnum_txt))) >= 0.6
            )
            if (
                skin >= 0.35
                and (conf < 0.55 or len(_alnum_txt) < 4)
                and not _looks_real_text
            ):
                skipped_face += 1
                if getattr(self, "debug", False):
                    print(f"        [dbg-drop] reason=FACE skin={skin:.2f} conf={conf:.2f} {text!r}")
                continue
            ...
            if box_a > page_a * 0.04 and skin >= 0.28 and len(text) < 8:
                skipped_face += 1
                if getattr(self, "debug", False):
                    print(f"        [dbg-drop] reason=FACE2 skin={skin:.2f} {text!r}")
                continue
            ...
            if kind == "junk" and conf < 0.55:
                empty_ocr += 1
                if getattr(self, "debug", False):
                    print(f"        [dbg-drop] reason=JUNK conf={conf:.2f} {text!r}")
                continue

            
            
            if kind != "promo":
                _stripped = self._strip_watermark_tokens(text)
                if _stripped != text:
                    if getattr(self, "debug", False):
                        print(f"        [dbg-wm] {text!r} → {_stripped!r}")
                    if len(re.sub(r"[^\w]", "", _stripped, flags=re.UNICODE)) < 3:
                        empty_ocr += 1
                        if getattr(self, "debug", False):
                            print(f"        [dbg-drop] reason=WATERMARK_ONLY {text!r}")
                        continue
                    text = _stripped
                    line_texts = [
                        _ln for _ln in (line_texts or [])
                        if self._strip_watermark_tokens(_ln)
                    ]
            ...
            if (not split_parts) and (not getattr(self, "_split_rejected_overlap", False)) and bw >= max(160, int(bh * 1.8)) and len(polys) >= 3:
                xs = []
                for poly in polys:
                    p = np.asarray(poly, dtype=np.float32).reshape(-1, 2)
            ...
            for part_txt, part_polys, part_rect in split_parts:
                ...
                _px, _py, _pw, _ph = [float(v) for v in part_rect]
                _px2, _py2 = _px + _pw, _py + _ph
                for _poly in (part_polys or []):
                    try:
                        _p = np.asarray(_poly, dtype=np.float32).reshape(-1, 2)
                    except Exception:
                        continue
                    if _p.size < 4:
                        continue
                    _px = min(_px, float(_p[:, 0].min()) - 3.0)
                    _py = min(_py, float(_p[:, 1].min()) - 3.0)
                    _px2 = max(_px2, float(_p[:, 0].max()) + 3.0)
                    _py2 = max(_py2, float(_p[:, 1].max()) + 3.0)
                _px = max(0.0, _px)
                _py = max(0.0, _py)
                _px2 = min(float(_piece_w), _px2)
                _py2 = min(float(y0 + _piece_h), _py2)
                _py = min(_py, max(0.0, _py2 - 8.0))
                part_rect = (int(_px), int(_py), max(8, int(_px2 - _px)), max(8, int(_py2 - _py)))
                ...
                _sp = det.get("split_parent")
                if _sp:
                    try:
                        setattr(_reg, "split_parent", [int(v) for v in list(_sp)[:4]])
                    except Exception:
                        pass
                regions.append(_reg)
        ...
        for r in regions:
            ...
            if boxes:
                ...
                if all_pts:
                    pts = np.vstack(all_pts)
                    if pts.size >= 4:
                        px0 = float(pts[:, 0].min()) - 2
                        py0 = float(pts[:, 1].min()) - 2
                        px1 = float(pts[:, 0].max()) + 2
                        py1 = float(pts[:, 1].max()) + 2
                        poly_rect = (px0, py0, max(1.0, px1 - px0), max(1.0, py1 - py0))

            def _clamp(qx0, qy0, qx1, qy1):
                cx0 = max(0, int(qx0)); cy0 = max(0, int(qy0))
                cx1 = min(vis_w - 1, int(qx1)); cy1 = min(vis_h - 1, int(qy1))
                return cx0, cy0, max(cx0 + 1, cx1), max(cy0 + 1, cy1)

            trust_poly = False
            if poly_rect is not None:
                ix0 = max(poly_rect[0], rx); iy0 = max(poly_rect[1], ry)
                ix1 = min(poly_rect[0] + poly_rect[2], rx + rw)
                iy1 = min(poly_rect[1] + poly_rect[3], ry + rh)
                inter = max(0.0, ix1 - ix0) * max(0.0, iy1 - iy0)
                union = poly_rect[2] * poly_rect[3] + rect_area - inter
                iou_v = inter / max(1.0, union)
                degenerate = poly_rect[2] < 4 or poly_rect[3] < 4
                trust_poly = (not degenerate) and (iou_v >= 0.25)

            if trust_poly:
                bx0, by0, bx1, by1 = _clamp(poly_rect[0], poly_rect[1],
                                            poly_rect[0] + poly_rect[2], poly_rect[1] + poly_rect[3])
                cv2.rectangle(vis, (bx0, by0), (bx1, by1), color, 2, cv2.LINE_AA)
                label_x, label_y = bx0, by0
            else:
                bx0, by0, bx1, by1 = _clamp(rx, ry, rx + rw, ry + rh)
                cv2.rectangle(vis, (bx0, by0), (bx1, by1), color, 2, cv2.LINE_AA)
                label_x, label_y = bx0, by0
                if poly_rect is not None:
                    ox0, oy0, ox1, oy1 = _clamp(poly_rect[0], poly_rect[1],
                                                poly_rect[0] + poly_rect[2], poly_rect[1] + poly_rect[3])
                    cv2.rectangle(vis, (ox0, oy0), (ox1, oy1), alt_color, 1, cv2.LINE_AA)
    ...
    def detect_regions_full(self, image: np.ndarray,
        ...
        for cy in (extra_cut_ys or []):
            try:
                ...
                continue
            if 200 <= cy_i <= h - 200:
                page_cuts.append(cy_i)
        page_cuts = sorted(set(page_cuts))

        
        
        chunk_ranges: List[Tuple[int, int, Tuple[str, ...]]] = []
        tech_cuts: List[int] = []
        cur = 0
        while True:
            if h - cur <= max_chunk + max(1, overlap):
                chunk_ranges.append((cur, h, ("top", "bottom") if (cur == 0 or cur in page_cuts) else ("bottom",)))
                break
            target = cur + max_chunk
            cands = [p for p in page_cuts if cur + 1200 <= p <= target + overlap]
            if cands:
                cut = min(cands, key=lambda p: abs(p - target))
            else:
                cut = target
                near = [p for p in page_cuts if cur + 1200 <= p and abs(p - cut) <= 320]
                if near:
                    cut = near[0]
                else:
                    tech_cuts.append(cut)
            if h - cut <= 900:
                
                top_edges = ("top", "bottom") if (cur == 0 or cur in page_cuts) else ("bottom",)
                chunk_ranges.append((cur, h, top_edges))
                break
            top_edges: Tuple[str, ...] = ("top",) if (cur == 0 or cur in page_cuts) else ()
            chunk_ranges.append((cur, min(cut + overlap, h), top_edges))
            cur = cut

        cut_ys = sorted(set(page_cuts + tech_cuts))
        if page_cuts:
            print(f"    [*] مرز صفحه داخل نوار: y={page_cuts} (برش تکه‌ها روی همین نقاط سنجاق شد)")

        all_regions: List[TextRegion] = []
        tasks = [(i, r[0], r[1], image, r[2]) for i, r in enumerate(chunk_ranges)]

        _order = (getattr(self, "extract_order", "auto") or "auto").lower().strip()
        if _order == "auto":
            _order = "bidi" if (self.max_workers > 1 and len(tasks) > 1) else "bottomup"
        if _order == "bidi" and (self.max_workers <= 1 or len(tasks) <= 1):
            _order = "bottomup"
        _order_fa = {"bottomup": "پایین به بالا", "topdown": "بالا به پایین", "bidi": "دوطرفه (بالا+پایین)"}
        print(f"    [*] ترتیب استخراج: {_order_fa.get(_order, _order)}")

        if _order in ("bottomup", "topdown") or self.max_workers <= 1 or len(tasks) <= 1:
            _seq = tasks if _order == "topdown" else list(reversed(tasks))
            seq_results = {}
            for t in _seq:
                seq_results[t[0]] = self._process_chunk_worker(t)
            for _ci in sorted(seq_results.keys()):
                all_regions.extend(seq_results[_ci])
        else:
            
            from collections import deque as _deque
            _task_q = _deque(tasks)
            _q_lock = threading.Lock()
            bidi_results = {}

            def _bidi_worker(from_front: bool) -> None:
                while True:
                    with _q_lock:
                        if not _task_q:
                            return
                        t = _task_q.popleft() if from_front else _task_q.pop()
                    bidi_results[t[0]] = self._process_chunk_worker(t)

            w_bidi = min(self.max_workers, len(tasks))
            with ThreadPoolExecutor(max_workers=w_bidi) as executor:
                _futs = [executor.submit(_bidi_worker, i % 2 == 0) for i in range(w_bidi)]
                for _f in _futs:
                    _f.result()
            for _ci in sorted(bidi_results.keys()):
                all_regions.extend(bidi_results[_ci])

        if getattr(self, "debug", False):
            print(f"    [dbg-raw] همهٔ OCR خام ({len(all_regions)} ناحیه) قبل از dedup:")
            for _r in all_regions:
                print(f"      [raw] ({_r.det_class}/{_r.det_confidence:.2f}) rect={_r.rect} {_r.source_text!r}")

        def _dbg_removed(before_list, after_list, tag):
            if not getattr(self, "debug", False):
                return
            from collections import Counter as _C
            cb = _C((r.source_text or "").replace("\n", " ")[:70] for r in before_list)
            ca = _C((r.source_text or "").replace("\n", " ")[:70] for r in after_list)
            removed = cb - ca
            added = ca - cb
            for txt, n in removed.items():
                print(f"    [dbg-{tag}] حذف×{n}: {txt!r}")
            for txt, n in added.items():
                print(f"    [dbg-{tag}] ادغام/جدید×{n}: {txt!r}")

        _n_split_merged = len(all_regions)
        all_regions = self._merge_split_text_regions(all_regions)
        if len(all_regions) < _n_split_merged:
            print(f"    [*] {_n_split_merged - len(all_regions)} نیمهٔ splitشدهٔ یک حباب دوباره به هم جوش خورد.")
        _dbg_removed(all_regions, self._dedupe_regions_by_rect(all_regions, iou_thresh=0.4), "stage0-iou")
        unique_regions = self._dedupe_regions_by_rect(all_regions, iou_thresh=0.4)
        before_contain = len(unique_regions)
        _sc1 = self._suppress_contained_regions(unique_regions, containment_thresh=0.70)
        _dbg_removed(unique_regions, _sc1, "stage1-contained")
        unique_regions = _sc1
        ...
        _mv1 = self._merge_vertically_split_regions(
            unique_regions, cut_ys=page_cuts, max_gap=90, edge_margin=150,
            min_x_overlap_ratio=0.25,
        )
        _dbg_removed(unique_regions, _mv1, "stage2-merge-pagecuts")
        unique_regions = _mv1
        if tech_cuts:
            _mv2 = self._merge_vertically_split_regions(
                unique_regions, cut_ys=tech_cuts, max_gap=55, edge_margin=90,
                min_x_overlap_ratio=0.30,
            )
            _dbg_removed(unique_regions, _mv2, "stage3-merge-techcuts")
            unique_regions = _mv2
        _ma1 = self._merge_adjacent_fragments(unique_regions)
        _dbg_removed(unique_regions, _ma1, "stage4-adjacent")
        unique_regions = _ma1
        if len(unique_regions) < before_merge:
            print(f"    [*] {before_merge - len(unique_regions)} حباب نصفه (برش chunk) به هم وصل شد.")
        _sd1 = self._suppress_text_dupes(unique_regions)
        _dbg_removed(unique_regions, _sd1, "stage5-textdup")
        unique_regions = _sd1
        _sc2 = self._suppress_contained_regions(unique_regions, containment_thresh=0.70)
        _dbg_removed(unique_regions, _sc2, "stage6-contained2")
        unique_regions = _sc2
        ...
        _n_pre = len(unique_regions)
        _sd2 = self._suppress_text_dupes(unique_regions)
        _dbg_removed(unique_regions, _sd2, "stage7-textdup2")
        unique_regions = _sd2
        _sc3 = self._suppress_contained_regions(unique_regions, containment_thresh=0.70)
        _dbg_removed(unique_regions, _sc3, "stage8-contained3")
        unique_regions = _sc3
        if len(unique_regions) < _n_pre:
            print(f"    [*] {_n_pre - len(unique_regions)} تکهٔ تکراری بعد از tail-rescue حذف شد.")
        ...
        if getattr(self, "debug", False):
            print(f"    [dbg-final] نتیجهٔ نهایی صفحه: {len(unique_regions)} ناحیه")
            for _r in unique_regions:
                print(f"      [final] #{_r.id} ({_r.kind}/{_r.bubble_style}) {_r.source_text!r}")
    ...
    def process_core(self, image: np.ndarray, extra_cut_ys: Optional[List[int]] = None) -> np.ndarray:
        ...
        def _rr_area(rr):
            return float(max(1, rr[2]) * max(1, rr[3]))
        ...
        sfx_extra: List[TextRegion] = []
        for s in sfx_regions:
            for d in dialogue_regions:
                if _rr_inter(s.rect, d.rect) >= 0.55 * _rr_area(s.rect):
                    sfx_extra.append(s)
                    break
        if sfx_extra:
            print(f"    [*] {len(sfx_extra)} SFX داخل حباب دیالوگ → پاک می‌شود (رندر نمی‌شود).")

        to_clean = dialogue_regions + promo_regions + sfx_extra
        cleaned_image = image
        clean_err = [None]  
        ...
        with ThreadPoolExecutor(max_workers=1) as clean_ex:
            ...
            if dialogue_regions:
                ...
                self._current_page_image = image
                self.translate_regions(dialogue_regions, page_image_bgr=image)
            else:
    ...
    @staticmethod
    def _merge_split_text_regions(regions: List[TextRegion]) -> List[TextRegion]:
        if len(regions) < 2:
            return regions

        def _sp(r):
            sp = getattr(r, "split_parent", None)
            if not sp:
                return None
            try:
                return [int(v) for v in list(sp)[:4]]
            except Exception:
                return None
        ...
        while changed:
            ...
            for i in range(len(regs)):
                ...
                a = regs[i]
                spa = _sp(a)
                if spa is None or a.kind != "dialogue":
                    out.append(a)
                    continue
                ax, ay, aw, ah = [float(v) for v in a.rect]
                for j in range(i + 1, len(regs)):
                    if skip[j]:
                        continue
                    b = regs[j]
                    spb = _sp(b)
                    if spb is None or b.kind != "dialogue":
                        continue
                    if any(abs(spa[t] - spb[t]) > 8 for t in range(4)):
                        continue
                    bx, by, bw, bh = [float(v) for v in b.rect]
                    yov = max(0.0, min(ay + ah, by + bh) - max(ay, by))
                    if yov < 0.55 * max(1.0, min(ah, bh)):
                        continue
                    gap = max(ax, bx) - min(ax + aw, bx + bw)
                    if gap > 0.15 * max(1.0, min(aw, bw)):
                        continue
                    
                    if (bx + bw / 2.0) < (ax + aw / 2.0):
                        first, second = b, a
                    else:
                        first, second = a, b
                    a = TextRegion(
                        id=-1,
                        boxes=list(a.boxes or []) + list(b.boxes or []),
                        source_text=((first.source_text or "").strip() + " "
                                     + (second.source_text or "").strip()).strip(),
                        rect=(int(min(ax, bx)), int(min(ay, by)),
                              int(max(ax + aw, bx + bw) - min(ax, bx)),
                              int(max(ay + ah, by + bh) - min(ay, by))),
                        angle=0.0,
                        kind="dialogue",
                        det_class=a.det_class if a.det_class == "text_bubble" else b.det_class,
                        det_confidence=max(float(a.det_confidence or 0), float(b.det_confidence or 0)),
                        bubble_style=a.bubble_style or b.bubble_style or "normal",
                        shape_type=getattr(a, "shape_type", "circle") or "circle",
                    )
                    try:
                        setattr(a, "split_parent", list(spa))
                    except Exception:
                        pass
                    skip[j] = True
                    changed = True
    ...
    def _suppress_contained_regions(
        ...
        def _texts_related(a: TextRegion, b: TextRegion) -> bool:
            ta, tb = _norm_tokens(a.source_text), _norm_tokens(b.source_text)
            if not ta or not tb:
                return False
            if ta <= tb or tb <= ta:
                return True
            inter = len(ta & tb)
            return (inter / max(1, len(ta))) >= 0.7 or (inter / max(1, len(tb))) >= 0.7

        def _text_len(r: TextRegion) -> int:
            return len(re.sub(r"[^a-z0-9]+", "", (r.source_text or "").lower()))
        ...
        for r in ordered:
            ...
            for k in list(kept):
                ...
                if contain_r_in_k >= containment_thresh:
                    ...
                    small_foreign = ar <= 0.5 * ak and not _texts_related(r, k)
                    if small_foreign:
                        continue
                    
                    
                    
                    if _texts_related(r, k):
                        if _text_len(r) > _text_len(k):
                            kept.remove(k)
                            continue
                        drop = True
                        break
                ...
                if contain_k_in_r >= containment_thresh:
                    small_foreign = ak <= 0.5 * ar and not _texts_related(k, r)
                    if small_foreign:
                        continue
                    if _texts_related(k, r):
                        if _text_len(k) > _text_len(r):
                            drop = True
                            break
                        kept.remove(k)
                        continue
    ...
    def _rescue_open_sentence_tails(self, image: np.ndarray,
        ...
        for r in regions:
            ...
            for o in regions:
                ...
                _allow = max(12, min(40, int(0.30 * oh)))
                if ox < x + w and ox + ow > x and oy >= y2 - _allow:
            ...
            _sx0 = max(0, x - int(0.20 * max(1, w)))
            _sx1 = min(image.shape[1], x + w + int(0.20 * max(1, w)))
            strip = image[strip_y0:limit, _sx0:_sx1]
            if strip.size == 0 or strip.shape[0] < 30:
                continue
            
            try:
                _g = cv2.cvtColor(strip, cv2.COLOR_BGR2GRAY)
                _med = float(np.median(_g))
                _ink = (_g < _med - 30) if _med > 135 else (_g > _med + 30)
                if float(np.count_nonzero(_ink)) < 0.004 * _ink.size:
                    continue
            except Exception:
                pass
            try:
            ...
            for b in (res or []):
                for ln in (b or []):
                    ...
                    if not (txt and conf >= 0.55 and cy > 6):
                        continue
                    
                    
                    _cx = float(box[:, 0].mean()) / 1.5 + strip_x0
                    _cy = cy + strip_y0
                    _swallowed = False
                    for o in regions:
                        if o is r:
                            continue
                        ox, oy, ow, oh = [int(v) for v in o.rect]
                        if (ox + 2) <= _cx <= (ox + ow - 2) and (oy + 2) <= _cy <= (oy + oh - 2):
                            _swallowed = True
                            break
                    if _swallowed:
                        continue
                    lines.append((cy, txt, box))
            if not lines:
                continue
            lines.sort(key=lambda t: t[0])
            add = " ".join(t2 for _, t2, _b in lines).strip()
            ...
            try:
                new_boxes = []
                for _cy, _txt, _box in lines:
                    p = np.asarray(_box, dtype=np.float32).reshape(-1, 2) / 1.5
                    p[:, 0] += strip_x0
                    p[:, 1] += strip_y0
                    new_boxes.append(p.astype(np.int32))
            except Exception:
                new_boxes = []
            if not new_boxes:
                continue
            all_pts = np.vstack([np.asarray(b).reshape(-1, 2) for b in new_boxes])
            nx0 = int(all_pts[:, 0].min()) - 3
            ny0 = int(all_pts[:, 1].min()) - 3
            nx1 = int(all_pts[:, 0].max()) + 3
            ny1 = int(all_pts[:, 1].max()) + 3
            
            ang = 0.0
            try:
                angs = []
                for _cy, _txt, _box in lines:
                    pb = np.asarray(_box, dtype=np.float32).reshape(-1, 2) / 1.5
                    if pb.shape[0] >= 2:
                        angs.append(float(np.degrees(np.arctan2(
                            pb[1][1] - pb[0][1], pb[1][0] - pb[0][0],
                        ))))
                if angs:
                    ang = float(np.mean(angs))
                    if abs(ang) < 6:
                        ang = 0.0
            except Exception:
                ang = 0.0
            out_regions.append(TextRegion(
                ...
                boxes=new_boxes,
                source_text=add,
                rect=(max(0, nx0), max(0, ny0), max(8, nx1 - nx0), max(8, ny1 - ny0)),
                angle=ang,
                kind="dialogue",
                det_class=getattr(r, "det_class", "text_bubble"),
                det_confidence=float(getattr(r, "det_confidence", 0.5) or 0.5),
                bubble_style="normal",
            ))
            try:
                setattr(out_regions[-1], "rescued_caption", True)
            except Exception:
                pass
            changed += 1
            print(f"    [tail-rescue] خط جاماندهٔ پایین حباب → ناحیهٔ مستقل: {add[:56]!r}")
        if changed:
    ...
    def _suppress_text_dupes(regions: List[TextRegion]) -> List[TextRegion]:
        ...
        def inter_frac(rr, krect):
            ix = max(0.0, min(rr[2], krect[2]) - max(rr[0], krect[0]))
            iy = max(0.0, min(rr[3], krect[3]) - max(rr[1], krect[1]))
            inter = ix * iy
            if inter <= 0:
                return 0.0
            ar = max(1.0, (rr[2] - rr[0]) * (rr[3] - rr[1]))
            ak = max(1.0, (krect[2] - krect[0]) * (krect[3] - krect[1]))
            return inter / min(ar, ak)
        ...
        def _fuzzy_cov(small_set, big_set):
            if not small_set:
                return 0.0
            matched = 0
            for t in small_set:
                if t in big_set:
                    matched += 1
                    continue
                if _SeqMatcher is not None and any(
                    _SeqMatcher(None, t, b).ratio() >= 0.75 for b in big_set
                ):
                    matched += 1
            return matched / float(len(small_set))
        ...
        for r in ordered:
            ...
            nr_ = nchar(r.source_text)
            rr = poly_rect(r) or (
                float(r.rect[0]), float(r.rect[1]),
                float(r.rect[0] + r.rect[2]), float(r.rect[1] + r.rect[3]),
            )
            drop = False
            for k in kept:
                kr = poly_rect(k) or (
                    float(k.rect[0]), float(k.rect[1]),
                    float(k.rect[0] + k.rect[2]), float(k.rect[1] + k.rect[3]),
                )
                containment = inter_frac(rr, kr)
                if containment < 0.15:
                    continue
                tk = toks(k.source_text)
                nk = nchar(k.source_text)
                ...
                cov_r = _fuzzy_cov(tr_, tk)
                cov_k = _fuzzy_cov(tk, tr_)
                if containment >= 0.25 and (cov_r >= 0.80 or cov_k >= 0.80) and min(len(tr_), len(tk)) >= 2:
                    drop = True
                    break
                
                cov_r_e = len(tr_ & tk) / float(max(1, len(tr_)))
                cov_k_e = len(tr_ & tk) / float(max(1, len(tk)))
                if containment >= 0.25 and (cov_r_e >= 0.75 or cov_k_e >= 0.75) and min(len(tr_), len(tk)) >= 3:
                    drop = True
                    break
                
                
                if containment >= 0.45:
                    if (cov_r >= 0.70 or cov_k >= 0.70) and min(len(tr_), len(tk)) >= 2:
                        drop = True
                        break
                    if _SeqMatcher is not None and nr_ and nk and (
                        _SeqMatcher(None, nr_, nk).ratio() >= 0.55
                    ):
                        drop = True
                        break
                
                if _SeqMatcher is not None and nr_ and nk:
                    if containment >= 0.25 and _SeqMatcher(None, nr_, nk).ratio() >= 0.72:
                        drop = True
                        break
            if drop:
                dropped += 1
    ...
    @staticmethod
    def _merge_two_regions(cur: TextRegion, b: TextRegion) -> TextRegion:
        cy1 = cur.rect[1]
        ...
        big = cur if (cur.rect[2] * cur.rect[3]) >= (b.rect[2] * b.rect[3]) else b
        mpoly = getattr(big, "mask_poly", None)
        if mpoly is not None:
            try:
                _pp = np.asarray(mpoly, dtype=np.float32).reshape(-1, 2)
                _pa = max(1.0, float((_pp[:, 0].max() - _pp[:, 0].min()) * (_pp[:, 1].max() - _pp[:, 1].min())))
                if _pa < 0.55 * float(nw * nh):
                    mpoly = None
            except Exception:
                mpoly = None
        return TextRegion(
            ...
            bubble_style=getattr(big, "bubble_style", "") or "normal",
            shape_type=getattr(big, "shape_type", "") or "circle",
            mask_poly=mpoly,
        )
    ...
    def _merge_adjacent_fragments(
        ...
        def x_ov(a: TextRegion, b: TextRegion) -> float:
            left = max(a.rect[0], b.rect[0])
            right = min(a.rect[0] + a.rect[2], b.rect[0] + b.rect[2])
            return max(0.0, right - left) / float(max(1.0, min(a.rect[2], b.rect[2])))
        ...
        for i, a in enumerate(ordered):
            ...
            while changed:
                ...
                for j, b in enumerate(ordered):
                    ...
                    if cur.kind != b.kind:
                        continue
                    if b.rect[1] >= cur.rect[1]:
                        top_r, bot_r = cur, b
                    else:
                        top_r, bot_r = b, cur
                    gap = bot_r.rect[1] - (top_r.rect[1] + top_r.rect[3])
                    if gap > max_gap or gap < -6:
                        continue
                    if x_ov(top_r, bot_r) < min_x_overlap:
                        continue
                    ca = top_r.rect[0] + top_r.rect[2] / 2.0
                    cb = bot_r.rect[0] + bot_r.rect[2] / 2.0
                    if abs(ca - cb) > max_center_drift * max(1.0, max(top_r.rect[2], bot_r.rect[2])):
                        continue
                    
                    
                    _tt = (top_r.source_text or "").strip()
                    _bt = (bot_r.source_text or "").strip()
                    if _tt and _bt and _tt[-1] in ".!?…۔" and _bt[:1].isupper():
                        continue
                    cur = MangaTranslator._merge_two_regions(cur, b)
                    used[j] = True
                    n_merged += 1
                    changed = True
            merged.append(cur)
        if n_merged:
            print(f"    [*] {n_merged} جفت نیمه‌حباب چسبیده (مرز نامرئی بین صفحات سورس) به هم وصل شد.")
        return merged
...
"""
    ...
    @staticmethod
    def _safe_extend_next(next_im, max_look: int = 1600, min_tail: int = 400,
                          min_tail_glue: int = 40):
        try:
            ih = int(next_im.shape[0])
            if ih < min_tail + 120:
                return None, next_im
            n = min(int(max_look), ih - min_tail)
            if n < min_tail_glue + 12:
                return None, next_im
            ink = MangaTranslator._row_ink_profile(next_im[:n + 12])
            if ink.shape[0] < 24:
                return None, next_im
            
            
            if float(ink[:24].max()) < 0.005:
                return None, next_im
            
            cut2 = None
            run = 0
            for y in range(min_tail_glue, int(ink.shape[0])):
                if float(ink[y]) < 0.003:
                    run += 1
                    if run >= 12:
                        cut2 = y - run + 6
                        break
                    ...
                    run = 0
            if not cut2 or cut2 < min_tail_glue or cut2 > max_look or (ih - cut2) < min_tail:
                return None, next_im
            
            g = cv2.cvtColor(next_im[:cut2 + 12], cv2.COLOR_BGR2GRAY) if next_im.ndim == 3 else next_im[:cut2 + 12]
            b = 10
            y0b = max(0, cut2 - b // 2)
            y1b = min(int(g.shape[0]), cut2 + b // 2 + 1)
            band_px = g[y0b:y1b]
            if band_px.size == 0:
                return None, next_im
            std = float(np.std(band_px))
            mean = float(np.mean(band_px))
            if not (std < 42.0 or mean > 225.0 or mean < 30.0):
                return None, next_im
            return np.ascontiguousarray(next_im[:cut2]), np.ascontiguousarray(next_im[cut2:])
        except Exception:
            return None, next_im

    @staticmethod
    def _safe_trim_prev(cur_im, max_look: int = 1600, min_tail: int = 400, min_keep: int = 900):
        try:
            ih = int(cur_im.shape[0])
            if ih < min_keep + min_tail + 120:
                return None
            ink = MangaTranslator._row_ink_profile(cur_im)
            
            content_idx = np.flatnonzero(ink >= 0.005)
            if content_idx.size == 0:
                return None
            last = int(content_idx[-1])
            if (ih - 1 - last) > int(max_look):
                return None
            ...
            for y in range(last, -1, -1):
                if float(ink[y]) < 0.003:
                    ...
                    run = 0
            if not cut or cut < min_keep or (ih - cut) < min_tail:
                return None
            g = cv2.cvtColor(cur_im[max(0, cut - 12): cut + 12], cv2.COLOR_BGR2GRAY) \
                if cur_im.ndim == 3 else cur_im[max(0, cut - 12): cut + 12]
            if g.size == 0:
                return None
            std = float(np.std(g))
        ...
        for f in image_files[start_idx:]:
            ...
            if current_pages and (current_h + h) > max_h:
                if current_h >= min_strip:
                    ...
                    _pending_tail = None
                    _head2, _rest = self._safe_extend_next(im)
                    if _head2 is not None:
                        current_bounds.append(current_h)
                        current_pages.append(_head2)
                        current_h += int(_head2.shape[0])
                        im = _rest
                        h = int(im.shape[0])
                    else:
                        
                        
                        _ink_next = MangaTranslator._row_ink_profile(im[:48])
                        _content_cut = bool(_ink_next.size) and float(_ink_next.max()) >= 0.005
                        _cut = self._safe_trim_prev(current_pages[-1]) if _content_cut else None
                        if _cut:
                            _tail = np.ascontiguousarray(current_pages[-1][_cut:])
                            current_pages[-1] = np.ascontiguousarray(current_pages[-1][:_cut])
                            current_h -= int(_tail.shape[0])
                            _pending_tail = _tail
                            print(f"        [>] مرز نوار: دُم {int(_tail.shape[0])}px به نوار بعدی منتقل شد (جلوگیری از نصف‌شدن حباب)")
                    emit_current(f"{len(current_pages)} صفحه (قبل از صفحه‌ی جدید)")
                    if _pending_tail is not None:
                        current_pages = [_pending_tail]
                        current_h = int(_pending_tail.shape[0])
                        current_bounds = []
        ...
        if page_w <= 1 or len(pending) <= 1:
            for item in pending:
                try:
                    ...
                    self._api_dead = True
                    print("    [!] ترجمه از کار افتاد (کلیدها تمام/مسدود). بقیه صفحات فقط پاکسازی و بدون ترجمه ذخیره میشوند.")
                    continue
...
def build_arg_parser():
    ...
    p.add_argument("--extract-order", choices=["auto", "bottomup", "topdown", "bidi"], default="auto",
                   help="ترتیب استخراج تکههای صفحه: auto (پیشفرض، دوطرفه) | bottomup | topdown | bidi. "
                        "روی سیستمهای ضعیف auto/بیدرش همیشه کار میکند.")
    p.add_argument("--strong-translation", action="store_true",
                   help="مدلهای Pro/قویتر اولویت میگیرند در کسکید Gemini (کندتر، کوتای بیشتر).")
    p.add_argument("--dry-run-fa", action="store_true",
                   help="OFFLINE test mode: skip translation API; use deterministic Persian sample texts.")
...
def main():
    ...
    if not unique_keys and provider != "ollama" and not getattr(args, "dry_run_fa", False):
    ...
    translator = MangaTranslator(
        ...
        dry_run_fa=bool(getattr(args, "dry_run_fa", False)),
        extract_order=getattr(args, "extract_order", "auto"),
    )
    if getattr(args, "strong_translation", False) and getattr(translator, "_model_cascade", None):
        _primary = translator._model_cascade[0]
        _pro = [m for m in translator._model_cascade[1:] if "pro" in m.lower()]
        _rest = [m for m in translator._model_cascade[1:] if "pro" not in m.lower()]
        if _pro:
            translator._model_cascade = [_primary] + _pro + _rest
            translator._model_index = 0
            translator.model_name = translator._model_cascade[0]
            print(f"[*] --strong-translation: کسکید قویتر شد: {', '.join(translator._model_cascade[:4])}")
    translator.api_timeout = float(getattr(args, "api_timeout", 120.0) or 120.0)
